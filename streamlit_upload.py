...
st.markdown(
    """
1. You upload an **evidence packet** (set of required CSV/Markdown files).
...
st.subheader("Upload Evidence Packet")
st.caption(
    "Upload the evidence files listed above. Optional files enable process/device checks."
)
uploaded_files = st.file_uploader(
    "Upload packet files (.md, .csv)",
    type=["md", "csv"],
    accept_multiple_files=True,
)

upload_root = Path.cwd() / ".uploaded_packets"
active_packet_dir = upload_root / "active_packet_ac_l1_b_1_i"

if uploaded_files:
    if active_packet_dir.exists():
        shutil.rmtree(active_packet_dir)
    active_packet_dir.mkdir(parents=True, exist_ok=True)
    for uploaded in uploaded_files:
        target_file = active_packet_dir / Path(uploaded.name).name
        target_file.write_bytes(uploaded.getbuffer())
    st.success(
        f"Uploaded {len(uploaded_files)} files to `{active_packet_dir}`."
    )

packet_dir = str(active_packet_dir.resolve())
packet_path = Path(packet_dir)
uploaded_names = {p.name for p in packet_path.glob("*")}
missing_required = [name for name in required_files if name not in uploaded_names]

st.markdown("**Required files**")
st.write(", ".join(f"`{name}`" for name in required_files))
st.markdown("**Optional files**")
st.write(", ".join(f"`{name}`" for name in optional_files))

if uploaded_files and missing_required:
    st.error(
        "Missing required evidence files: "
        + ", ".join(f"`{name}`" for name in missing_required)
    )
elif not uploaded_files:
    st.info("Upload your evidence packet files to enable preview and verification.")
...
if show_preview:
    ...
    if not uploaded_files:
        st.info("Upload files first to preview evidence.")
    else:
...
if run_verification:
    ...
    if not uploaded_files:
        st.error("Upload an evidence packet before running verification.")
        st.stop()
    if missing_required:
        st.error(
            "Cannot verify. Missing required evidence files: "
            + ", ".join(f"`{name}`" for name in missing_required)
        )
        st.stop()
    try:
